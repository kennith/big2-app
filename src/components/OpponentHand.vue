...
<template>
  ...
  <div class="flex flex-col items-center gap-1 transition-all duration-300 relative w-[96px] min-[370px]:w-[108px] sm:w-[135px] md:w-[150px] flex-shrink-0">
    ...
    >
      ...
      <div class="relative">
        <div
          class="w-9 h-9 sm:w-12 sm:h-12 rounded-full bg-slate-800 border-2 flex items-center justify-center text-lg sm:text-2xl shadow-inner relative"
        ...
        <div
          class="absolute -bottom-1 -right-1 sm:-bottom-1.5 sm:-right-1.5 px-1.5 sm:px-2 py-0.2 rounded-full text-[10px] sm:text-[11px] font-black shadow-md flex items-center gap-0.5 border"
          :class="[
            isLowCards
      ...
      <div class="my-0.5 text-center w-full overflow-hidden">
        <div class="font-bold text-[11px] sm:text-sm text-slate-100 truncate px-0.5">
          {{ player.name }}
        </div>
        <div class="flex items-center justify-center gap-1 mt-0.5">
          <span
            class="text-[9px] sm:text-[10px] px-1 sm:px-1.5 py-0.2 rounded border font-medium tracking-wider truncate max-w-full"
            :class="personalityColor"
          >
            {{ personalityLabel }}
          </span>
        </div>
        <div class="text-[10px] sm:text-[11px] text-slate-400 mt-0.5 font-mono">
          {{ t.totalPts }}: <span class="text-amber-300 font-semibold">{{ matchScore }}</span>
        </div>
      </div>

      <!-- Turn / Thinking / Action Bubble -->
      <div class="min-h-[18px] sm:min-h-[22px] flex items-center justify-center w-full">
        <div v-if="isActive && isThinking" class="flex items-center justify-center gap-1 text-[10px] sm:text-[11px] text-amber-300 font-medium">
        ...
        <div
          ...
          class="text-[10px] sm:text-[11px] px-1.5 sm:px-2 py-0.2 rounded-full bg-slate-800 border border-slate-700 text-slate-200 truncate max-w-full text-center"
        >
          {{ lastActionText }}
        </div>
      </div>
    ...
    <div
      ...
      class="flex relative items-center justify-center h-10 sm:h-12 mt-0.5"
    >
      <div
